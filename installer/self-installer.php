...
?>
      ...
      <p>Définissez ci dessous le dossier parent où MedShakeEHR doit être installé.<br>
        <strong> - Cet emplacement ne doit pas être accessible au réseau (ex:/opt)</strong><br>
        <strong> - L'utilisateur www-data doit avoir les droits d'écriture sur cet emplacement, ainsi que sur le dossier dans lequel est vous avez placé self-installer.php</strong> (fichier en cours d'exécution).</p>
      <form	action="<?=$_SERVER['REQUEST_URI']?>" method="post" style="margin-top:50px;">
        ...
        <button type="submit" class="btn" onclick="this.style.display='none';document.querySelector('.svgcontainer').className+=' svganim';">Suivant</button>
