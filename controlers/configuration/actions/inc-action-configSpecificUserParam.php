...
$prevData=msSQL::sql2tabKey("SELECT od.typeID AS k, dt.formType AS type, od.id AS Id, od.value AS Value 
        ...
        LEFT JOIN data_types AS dt ON od.typeID=dt.id and od.toID='".$_POST['userID']."' and od.outdated='' and od.deleted=''
...
if (is_array($_POST)) {
    foreach ($_POST as $k => $v) {
        ...
        if (is_numeric($typeID) and is_numeric($_POST['userID']) and 
          ((!array_key_exists($typeID, $prevData) and $v) or (array_key_exists($typeID, $prevData) and $v!=$prevData[$typeID]))) {
            $objet = new msObjet();
            $objet->setFromID($p['user']['id']);
            $objet->setToID($_POST['userID']);
            $id=$objet->createNewObjet($typeID, $v);
            if (array_key_exists($typeID, $prevData) and $prevData[$typeID]['type']="password" and $v!=$prevData[$typeID]['Value']) {
