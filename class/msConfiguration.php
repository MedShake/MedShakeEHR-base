...
class msConfiguration
{
    ...
    public static function listAvailableParameters($user) {
        ...
        self::$_usersParams=self::getUserParamatersForUser($user['id']);
    ...
    public static function getAllParametersForUser($user) {
        $defaultParams=msSQL::sql2tabKey("SELECT name, value FROM configuration WHERE level='default'", 'name');
        $moduleParams=msSQL::sql2tabKey("SELECT name, value FROM configuration WHERE level='module' and module='".$user['module']."'", 'name');
        $userParams=msSQL::sql2tabKey("SELECT name, value FROM configuration WHERE level='user' and toID='".$user['id']."'", 'name');
    ...
    // $userID: id du user
    // return array: tableau des paramètres
    public static function getUserParamatersForUser($userID) {
        $userParams=msSQL::sql2tabKey("SELECT name, value FROM configuration WHERE level='user' AND toID='".$userID."'", 'name');
        if (!is_array($userParams)) {
            return array();
        }
        $catTypeDefault=msSQL::sql2tabKey("SELECT name, cat, type, value FROM configuration WHERE level='default' AND name in ('".implode("','",array_keys($userParams))."')", 'name');
        foreach ($userParams as $k=>$v) {
            ...
            if (strpos(strtolower($k), 'password')!==false) {
    ...
    //obtenir un paramètre de configuration pour un user, éventuellement à sa valeur par défaut
    // $name nom du paramètre désiré
    // $user[]: user
    // return value: valeur du paramètre
    public static function getParameterValue($name, $user) {
        if (strpos(strtolower($name), 'password')!==false) {
            $param=msSQL::sql2tabKey("SELECT level, CONVERT(AES_DECRYPT(UNHEX(value),@password), CHAR) FROM configuration WHERE name='".$name."' AND 
