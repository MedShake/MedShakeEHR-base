    ...
    */
      private function _formatEvent($e)
      {
          ...
          if (!isset($this->_tabTypeRdv)) {
              if (is_file($p['config']['homeDirectory'].'config/agendas/typesRdv'.$this->_userID.'.yml')) {
                  $this->_tabTypeRdv=Spyc::YAMLLoad($p['config']['homeDirectory'].'config/agendas/typesRdv'.$this->_userID.'.yml');
              } else {
 ...
 */
    ...
    public static function getRdvTypes($userID) {
        global $p;
        if(is_file($p['config']['homeDirectory'].'config/agendas/typesRdv'.$userID.'.yml')) {
          return Spyc::YAMLLoad($p['config']['homeDirectory'].'config/agendas/typesRdv'.$userID.'.yml');
        } else {
          return array(
            '[C]'=> array(
              'descriptif'=>'Consultation',
