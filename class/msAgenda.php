...
*/
  public function getHistoriquePatient($limit=10)
  {
      ...
      $data['historique']=msSQL::sql2tab("select DATE_FORMAT(start, '%Y %m %d - %H:%i') as start, DATE_FORMAT(start, '%Y%m%d') as dateJump, DATE_FORMAT(start, '%Y-%m-%dT%TZ') as dateiso, type, statut, absente, motif, userid as agendaID from agenda where patientid='".$this->_patientID."' order by start desc limit $limit");
